...
// Siri Orb — adapted from smoothui to this repo. A rotating multi-stop
// conic-gradient blob with a gooey blur/contrast pass; self-contained styles
// via an inline <style> (uses @property --angle). Ships its own reduced-motion
// guard (freezes the rotation).
...
export default function SiriOrb({ size = '192px', className, colors, animationDuration = 20 }: SiriOrbProps) {
  const defaultColors = {
  ...
  return (
    <div className={`siri-orb${className ? ` ${className}` : ''}`} aria-hidden style={style}>
      <style>{`
        ...
        .siri-orb::after {
          ...
          -webkit-backdrop-filter: blur(calc(var(--blur-amount) * 2)) contrast(calc(var(--contrast-amount) * 2));
          backdrop-filter: blur(calc(var(--blur-amount) * 2)) contrast(calc(var(--contrast-amount) * 2));
          mix-blend-mode: overlay;
        }
        .siri-orb[style*="--mask-radius: 0%"]::after { -webkit-mask-image: none; mask-image: none; }
